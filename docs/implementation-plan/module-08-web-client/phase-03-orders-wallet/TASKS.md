...
### Orders List Page (P0)
[x] 1.  Build /orders page with "As Client" / "As Provider" tabs
[x] 2.  Order cards: listing title, counterparty, status badge, amount, last activity
[x] 3.  "Requires Action" indicator (orange) on orders needing input
[ ] 4.  Filter: status, date range
[x] 5.  Empty state per tab: client → "Browse services"; provider → "Make sure your listings are active"

### Order Detail Page (P0)
[x] 6.  Build /orders/:id page (renders differently for client vs provider)
[x] 7.  Status timeline: horizontal (desktop) / vertical (mobile)
[x] 8.  Listing summary, counterparty card, message thread, audit trail
[x] 9.  Client actions by state:
[x] 10.   draft → "Waiting for provider" / Cancel
[x] 11.   accepted → "Pay Now" (Paystack) / "Pay from Wallet (₦X balance)"
[x] 12.   delivered → "Approve & Release" / "Request Revision" / "Raise Dispute"
[ ] 13.   completed → "Rate this experience"
[x] 14. Provider actions by state:
[x] 15.   draft → "Accept Order" (with fee breakdown) / "Decline"
[x] 16.   in_progress → "Submit Deliverables"
...
[x] 19. Approve modal: MKT-M05 (confirmation with amount; feels weighty — money moves)
[x] 20. Dispute modal: MKT-M06 (category, description, file attachments)
...
### Wallet Page (P0)
[x] 26. Build /wallet page
[x] 27. Balance display: "Available: ₦X" and "In Active Orders: ₦Y" (NOT "Locked in Escrow")
[x] 28. Tooltip on "In Active Orders": "Held safely until work is approved"
[x] 29. Transaction history: date, description, amount (+/-), running balance
[ ] 30. Filter: type (all/funded/earned/withdrawn/orders), date range
[x] 31. "Add Funds" → FIN-M01 → Paystack checkout → FIN-M04 success
[x] 32. "Withdraw" → FIN-M02 → bank account selector → FIN-M03 (add bank) → processing
...
### Notifications (P0)
[x] 35. Build /notifications page: chronological, grouped (today/this week/earlier)
[x] 36. Notification bell with unread count badge in header
[ ] 37. Bell dropdown: last 5-10 notifications + "View All"
[x] 38. Each notification links to the relevant page
[x] 39. Mark individual and "mark all as read"
[ ] 40. In-app notification delivery via WebSocket (same connection as messages)

### Account Settings (P0)
[x] 41. Build /settings page
[x] 42. Sections: security, payment methods, identity verification status, tier progress, privacy
...
### Messaging (P0)
[x] 47. Build /messages page: conversation list, search
[x] 48. Build /messages/:conversationId page: real-time thread
